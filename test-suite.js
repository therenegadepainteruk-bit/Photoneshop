...
function test(name, fn) {
  ...
  catch (e) { console.log('  \u2717 ' + name + '\n      ' + (e && e.message ? e.message : e)); failed++; failures.push(name); }
}
...
function assertEqual(a, b, msg) { if (a !== b) throw new Error((msg || 'not equal') + ' (got ' + a + ', expected ' + b + ')'); }
...
const documentStub = {
  getElementById: (id) => (id in htParams ? { value: htParams[id], classList: { add(){}, remove(){}, toggle(){} } } : null),
};
...
const sandbox = {
  ...
  Uint8Array, Uint8ClampedArray, Float32Array, Array, Math, JSON, Date, Promise, Object, Number, String, Boolean, Error,
...
function loadIsolated(rel) {
  ...
  const sb = {
    ...
    window: {}, document: { getElementById: () => elStub() },
    console: { log() {}, warn() {}, error() {} }, setTimeout: (fn) => fn(),
    Math, JSON, Date, Promise, Object, Array, Number, String, Boolean, Error, parseFloat, parseInt,
  };
...
(async function run() {
  ...
  test('all six real source files loaded without throwing', () => assert(loadOK, 'one or more source files threw on load'));
  ...
  test('runAllChecks returns {allPass, results} shape on real call', () => {
    ...
    assert(entries.every(e => e && typeof e.pass === 'boolean'), 'a check result lacks a boolean .pass');
  });
  ...
  test('HEADLINE GUARD: guard() PASSES a real UXP RGB document (doc.mode="RGBColorMode", no colorModel)', () => {
    ...
    assert(api.ctx.guard() === true, 'guard() blocked a real RGB document \u2014 the colorModel bug would stop the halftone button on every real doc');
  });
  ...
  test('REGRESSION GUARD: no source file gates on doc.colorModel equality', () => {
    ...
    for (const f of files) {
      ...
      assert(!/doc\.colorModel\s*(===|!==)\s*['"]RGB['"]/.test(src),
  ...
  test('Benchmark.start/end computes duration and throughput on real timer', () => {
    ...
    // burn a little time
    let x = 0; for (let i = 0; i < 1e5; i++) x += i;
    bench.end(50000);
  ...
  test('tiledHalftoneRender (exported) produces real tiles with ink for a dark source', async () => {
    const w = 300, h = 300, depth = 4; // > TILE_SIZE so multiple tiles are produced
    const src = new Uint8Array(w * h * depth);
    for (let i = 0; i < w * h; i++) { src[i*4+3] = 255; } // opaque, tone=0 (darkest) => max dot
    const tiles = await W.PhotoneshopHalftoneTiled.tiledHalftoneRender(
      src, depth, w, h, { lpi: 45, angle: 45, dotGain: 0, dpi: 300, inkR: 0, inkG: 0, inkB: 0 }, () => {});
    assert(Array.isArray(tiles) && tiles.length > 1, 'expected multiple tiles, got ' + (tiles && tiles.length));
    let ink = 0;
    for (const t of tiles) { for (let i = 3; i < t.pixels.length; i += 4) if (t.pixels[i] === 255) ink++; }
  ...
  test('applyHalftoneTiled THROWS (experimental, not a silent fake success)', async () => {
    let threw = false;
    try { await W.PhotoneshopHalftoneTiled.applyHalftoneTiled({}, {}, () => {}); }
    catch (e) { threw = true; assert(/experimental|not implemented/i.test(e.message), 'wrong error: ' + e.message); }
  ...
  test('integration entry point is exported as a function', () => {
    assert(W.PhotoneshopHalftoneIntegrated && typeof W.PhotoneshopHalftoneIntegrated.applyHalftone === 'function',
  ...
  test('REGRESSION GUARD: no reference to an unassigned window.PhotoneshopHalftone global', () => {
    ...
    if (refsBare) {
      ...
      assert(assigns, 'halftone.js references window.PhotoneshopHalftone in code but never assigns it (the v5.2 dead-global bug)');
    }
    assert(true);
  });
  ...
  await testAsync('FUNCTIONAL: applyHalftone reads real pixels and writes a real halftone (end-to-end)', async () => {
    ...
    for (let y = 0; y < h; y++) {
      ...
      for (let x = 0; x < w; x++) {
        const i = (y * w + x) * 4;
        grad[i] = tone; grad[i+1] = tone; grad[i+2] = tone; grad[i+3] = 255;
      }
    }

    let getCalled = false, putCalled = false, putBounds = null, capturedOut = null;
    W.app = { activeDocument: { resolution: 300, width: w, height: h, mode: 'RGBColor', bitsPerChannel: 8,
    ...
    W.imaging = {
      ...
      createImageDataFromBuffer: async (buf, meta) => { capturedOut = buf; return { dispose() {} }; },
      putPixels: async (args) => { putCalled = true; putBounds = args && args.targetBounds; },
    };

    const variation = await W.PhotoneshopHalftoneIntegrated.applyHalftone(12345, 1, () => {});

    assert(getCalled, 'getPixels was never called \u2014 render did not read the layer (v5.2 regression: threw before reading)');
    ...
    let ink = 0; for (let i = 3; i < capturedOut.length; i += 4) if (capturedOut[i] === 255) ink++;
  ...
  await testAsync('FUNCTIONAL: integration surfaces render errors (does not swallow)', async () => {
    ...
    W.imaging = {
      getPixels: async () => { throw new Error('simulated getPixels failure'); },
      createImageDataFromBuffer: async () => ({ dispose() {} }),
      putPixels: async () => {},
    };
    let threw = false, msg = '';
    try { await W.PhotoneshopHalftoneIntegrated.applyHalftone(1, 1, () => {}); }
  ...
  test('rgbToCmyk: pure black -> k=1, c=m=y=0', () => {
    ...
    assertEqual(Math.round(r.c * 100), 0, 'c'); assertEqual(Math.round(r.m * 100), 0, 'm'); assertEqual(Math.round(r.y * 100), 0, 'y');
  });
  test('rgbToCmyk: pure white -> c=m=y=k=0', () => {
    const r = context.rgbToCmyk(255, 255, 255);
    assertEqual(Math.round(r.c * 100), 0, 'c'); assertEqual(Math.round(r.m * 100), 0, 'm');
    assertEqual(Math.round(r.y * 100), 0, 'y'); assertEqual(Math.round(r.k * 100), 0, 'k');
  });
  test('rgbToCmyk: pure red -> m=y=1, c=k=0 (red = magenta+yellow)', () => {
    const r = context.rgbToCmyk(255, 0, 0);
    assertEqual(Math.round(r.c * 100), 0, 'c'); assertEqual(Math.round(r.m * 100), 100, 'm');
    assertEqual(Math.round(r.y * 100), 100, 'y'); assertEqual(Math.round(r.k * 100), 0, 'k');
  });
  ...
  test('CMYK_ANGLES uses the classic 15/75/0/45 prepress convention (anti-moiré spacing)', () => {
    ...
    assert(/CMYK_ANGLES\s*=\s*\{\s*c:\s*15,\s*m:\s*75,\s*y:\s*0,\s*k:\s*45\s*\}/.test(src),
  ...
  test('REGRESSION GUARD: writeChannelLayer upscales to full document size (canvas-coverage fix)', () => {
    const src = fs.readFileSync(path.join(ROOT, 'engines/separation.js'), 'utf8');
    assert(/function writeChannelLayer[\s\S]{0,400}upscaleNearest/.test(src),
  ...
  test('REGRESSION GUARD: no CSS color assignment uses the invalid let(--x) typo for var(--x)', () => {
    ...
    for (const f of files) {
      const src = fs.readFileSync(path.join(ROOT, f), 'utf8');
      assert(!/\blet\(--/.test(src), f + ' contains the invalid CSS "let(--x)" typo (should be "var(--x)") — colour styling silently no-ops');
    }
  });
