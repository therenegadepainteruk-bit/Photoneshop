...
async function runPrintDoctor() {
  ...
  try {
    ...
    if (!doc) { setStatus("No active document", "error"); return; }
    ...
    try {
      await modal("Photoneshop: coverage sample", async function() {
        ...
        for (let i = 0; i < buf.length; i += comps) {
          let r = buf[i], g = buf[i+1] || 0, b = buf[i+2] || 0;
    ...
    } catch (e) { /* pixel sampling failed — skip coverage */ }
    ...
    _doctorResults = { score: score, issues: issues, warnings: warnings, info: info,
                       fixes: fixes, recommended: recommended, inkCoverage: inkCoverage,
                       colorVariance: colorVariance, estMinutes: estMinutes, estSecs: estSecs,
                       inkMl: inkMl, inkCost: inkCost };

    renderPrintDoctor(_doctorResults);
    updateFixAvailability();
    setStatus("Print Doctor: " + score.toFixed(1) + "/10", score >= 8 ? "success" : "warning");
  } catch (e) { setStatus("Analysis error: " + (e.message || e), "error"); }
}
...
function renderPrintDoctor(r) {
  ...
  let html = "";
  r.issues.forEach(function(i)   { html += '<div class="dr-row dr-err">\u2715  ' + i + '</div>'; });
  r.warnings.forEach(function(w) { html += '<div class="dr-row dr-warn">\u26a0  ' + w + '</div>'; });
  r.info.forEach(function(i)     { html += '<div class="dr-row dr-ok">\u2713  ' + i + '</div>'; });
  html += '<div class="dr-stats">';
  html += '<div class="dr-stat"><div class="dr-stat-v">' + r.estMinutes + 'm ' + r.estSecs + 's</div><div class="dr-stat-l">Est. print</div></div>';
  html += '<div class="dr-stat"><div class="dr-stat-v">' + r.inkCoverage + '%</div><div class="dr-stat-l">Coverage</div></div>';
  html += '<div class="dr-stat"><div class="dr-stat-v">' + r.inkCost + '</div><div class="dr-stat-l">Est. ink</div></div>';
  html += '</div>';
  detailEl.innerHTML = html;

  if (recEl) {
    recEl.innerHTML = '<div class="dr-rec-label">Recommended method</div><div class="dr-rec-val">' + r.recommended + '</div>';
    if (r.fixes.length > 0) {
      recEl.innerHTML += '<div class="dr-fixes-label">Suggested fixes</div>';
      r.fixes.forEach(function(f) { recEl.innerHTML += '<div class="dr-fix">' + f + '</div>'; });
    }
  }
}
...
async function fixUpscale() {
  ...
  try {
    let doc = window.app.activeDocument;
    if (!doc) return;
    if (doc.resolution >= 300) { setStatus("Already 300 DPI or higher", "info"); return; }
...
async function fixResize() {
  ...
  try {
    ...
    if (!w && !h) { setStatus("Enter a width or height in inches", "warning"); return; }
...
async function fixEnhance() {
  ...
  try {
    ...
    await runNonDestructive("Auto Enhanced", [
      { _obj: "autoTone" },
      { _obj: "autoContrast" },
      { _obj: "autoColorCorrection" }
    ], "Print Doctor");
    ...
    try {
      ...
      setStatus("Auto enhance applied — new layer", "success");
    } catch (e2) { setStatus("Error: " + (e2.message || e2), "error"); }
  }
}
...
function updateFixAvailability() {
  ...
  if (!doc) {
    [upBtn, flatBtn].forEach(function(b) { if (b) b.disabled = true; });
  ...
  if (upBtn) {
    ...
    upBtn.textContent = res >= 300 ? "Already 300 DPI (" + res + " DPI)" : "Upscale to 300 DPI (" + res + " \u2192 300)";
  }
  if (flatBtn) {
...
async function runDeepAnalysis() {
  ...
  if (body) {
    body.innerHTML = '<div class="deep-progress">'
  ...
  try {
    ...
    if (gamutRisk > 3) {
      ...
      colourReason = gamutRisk + "% of pixels are highly saturated colours that fall outside typical CMYK gamut. Converting now would dull these — keep RGB for DTG/DTF and only convert if your screen print workflow requires spot/CMYK separations.";
    } else if (colorCount < 40) {
      colourRec = "Safe to convert to CMYK";
      colourReason = "Low colour complexity (" + colorCount + " clusters) with minimal out-of-gamut colour. CMYK conversion should be safe for proofing or process printing.";
    } else {
      colourRec = "RGB recommended";
      colourReason = "Colour palette is broad. RGB preserves the widest range for DTG/DTF; convert to CMYK only at final separation if needed.";
    }

    let bandingRisk = "Low";
    let midtoneRange = histo.slice(64, 192);
    let nonZeroMidtones = midtoneRange.filter(function(v) { return v > 0; }).length;
    ...
    renderDeepResults({
      scanW: scanW, scanH: scanH, pixelsScanned: pixelsScanned,
      fullW: doc.width, fullH: doc.height,
      edgeDensity: edgeDensity, colorCount: colorCount,
      lpi: lpi, angle: angle, pattern: pattern, lpiReason: lpiReason,
      colourRec: colourRec, colourReason: colourReason,
      bandingRisk: bandingRisk, clippingNote: clippingNote,
      clipShadow: clipShadow, clipHighlight: clipHighlight, gamutRisk: gamutRisk
    });

  } catch (e) {
    if (body) body.innerHTML = '<p class="hint" style="color:var(--err)">Deep analysis failed: ' + (e.message || e) + '</p>';
  }
}
...
function renderDeepResults(r) {
  ...
  html += '<p class="hint">Scanned ' + r.scanW + ' \u00d7 ' + r.scanH + ' px ('
    + r.pixelsScanned.toLocaleString() + ' pixels'
    + (coveragePct < 100 ? ', a high-resolution sample of the full ' + r.fullW + '\u00d7' + r.fullH + ' image \u2014 full pixel-by-pixel scanning on large files would freeze Photoshop' : ', full resolution')
    + ').</p>';

  html += '<div class="deep-section"><div class="deep-label">Recommended Halftone</div>'
    + '<div class="deep-rec"><div class="deep-rec-title">' + r.pattern + ' \u2014 ' + r.lpi + ' LPI at ' + r.angle + '\u00b0</div>'
    + '<div class="deep-rec-sub">' + r.lpiReason + '</div></div>'
    + '<button class="btn" id="applyDeepHalftone" style="margin-top:6px;">Apply these halftone settings</button>'
    + '</div>';

  html += '<div class="deep-section"><div class="deep-label">Recommended Colour Mode</div>'
    + '<div class="deep-rec"><div class="deep-rec-title">' + r.colourRec + '</div>'
    + '<div class="deep-rec-sub">' + r.colourReason + '</div></div>'
    + '</div>';
  ...
  r.clippingNote.forEach(function(n) { html += '<div class="deep-row warn">\u26a0 ' + n + '</div>'; });
  if (r.bandingRisk !== "Low") html += '<div class="deep-row warn">\u26a0 Banding risk: ' + r.bandingRisk + '</div>';
  html += '</div>';

  html += '<div class="deep-stats-grid">'
    + '<div class="deep-stat"><div class="deep-stat-v">' + (r.edgeDensity * 100).toFixed(1) + '%</div><div class="deep-stat-l">Edge density</div></div>'
    + '<div class="deep-stat"><div class="deep-stat-v">' + r.colorCount + '</div><div class="deep-stat-l">Colour clusters</div></div>'
    + '<div class="deep-stat"><div class="deep-stat-v">' + r.clipShadow + '%</div><div class="deep-stat-l">Shadow clip</div></div>'
    + '<div class="deep-stat"><div class="deep-stat-v">' + r.clipHighlight + '%</div><div class="deep-stat-l">Highlight clip</div></div>'
    + '</div>';
...
async function setColourMode(mode) {
  ...
  try {
    ...
    await modal("Photoneshop: colour mode", async function() {
      await bp([{ _obj: "convertMode", to: { _enum: "colorSpace", _value: mode === "CMYK" ? "CMYKColorMode" : "RGBColorMode" } }]);
    });
...
function updateColourModeToggle() {
  ...
  if (!hasDoc()) { rgbBtn.classList.remove("on"); cmykBtn.classList.remove("on"); return; }
...
async function runProductionCheck() {
  if (!hasDoc()) { setStatus("Open an image in Photoshop first", "warning"); return; }
  ...
  if (detailEl) {
    let html = "";
    r.issues.forEach(function(i)   { html += '<span style="color:var(--err)">\u2715 ' + i + '</span><br>'; });
    r.warnings.forEach(function(w) { html += '<span style="color:var(--warn)">\u26a0 ' + w + '</span><br>'; });
    r.info.forEach(function(i)     { html += '<span>\u2713 ' + i + '</span><br>'; });
    detailEl.innerHTML = html;
  }
}
