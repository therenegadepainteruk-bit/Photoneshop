...
let TAB_GROUPS = ["DesignStudio","DesignStudioThreshold","DesignStudioTone","DesignStudioVintage",
  "HalftoneDots","CleanupFix","GarmentOptimise","ColoursReduce","WhiteInkUnderbase",
  "SeparationRecolour","DTGOptimise","DTFOptimise"];
...
function recordAction(type, name, layerId) {
  ...
  if (_psHistory.length > HISTORY_CAP) { _psHistory.shift(); }
}
function getLayerTarget()         { return _layerTarget; }
function setLayerTarget(t)        { _layerTarget = t; }

async function undoLast() {
  if (!guard()) return;
  if (_psHistory.length === 0) { setStatus("Nothing to undo", "info"); return; }
...
async function groupSelectedInto(group, layerId) {
  try {
    ...
    for (let i = 0; i < layers.length; i++) {
      let L = layers[i];
      if (L.name === group && L.layers && L.layers.length >= 0) { exists = true; break; }
    }
    // FIX 2.9: Use explicit _id reference if layerId provided, otherwise use selection
    const targetRef = layerId ? { _ref: "layer", _id: layerId } : { _ref: "layer", _enum: "ordinal", _value: "targetEnum" };
    if (!exists) {
      await bp([{ _obj: "make", _target: [{ _ref: "layerSection" }],
        from: targetRef,
        name: group }]).catch(async function() {
          await bp([{ _obj: "groupEvent" }]).catch(function() {});
          await bp([{ _obj: "set",
            _target: [targetRef],
            to: { _obj: "layer", name: group } }]).catch(function() {});
        });
    } else {
      await bp([{ _obj: "move",
        _target: [targetRef],
        to: { _ref: "layer", _name: group } }]).catch(function() {});
    }
  } catch (e) { /* grouping is best-effort; never block the op */ }
}
...
async function updateCoverage() {
  ...
  try {
    ...
    let doc = window.app.activeDocument;
    if (!doc || !doc.activeLayer) { out.textContent = "—"; return; }
...
let SLIDER_DEFAULTS = {
  thresh:128, exposure:0, highlight:0, shadow:0, bright:0, contrast:0, blur:0,
  halftone:100, htSize:8, htAngle:45, dotGain:0, lpi:45, htColor:"#000000",
  fade:0, distress:0, bleed:0, grain:0,
  wDensity:100, wChoke:0, wFeather:0, wHl:0,
  dtgInk:0, dtgWhite:0, dtgDetail:0,
  dtfCol:0, dtfInk:0, dtfSharp:0
};
