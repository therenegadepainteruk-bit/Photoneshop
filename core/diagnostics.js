...
function diagReset() { _diagLog = []; const el = document.getElementById("diagLog"); if (el) el.textContent = ""; }

function logImageDataShape(label, imgData) {
  let keys = [];
  try { keys = Object.keys(imgData); } catch (e) {}
  diagLog(label + " -> width=" + imgData.width + " height=" + imgData.height +
    " components=" + imgData.components +
    " colorSpace=" + (imgData.colorSpace !== undefined ? imgData.colorSpace : "(not exposed)") +
    " keys=[" + keys.join(",") + "]");
}
...
async function diagTest1() {
  ...
  if (!guard()) { diagLog("SKIPPED: no document open"); return false; }
  const w = 100, h = 100, expected = w * h * 4;
  const buf = new Uint8Array(expected);
  for (let i = 0; i < w * h; i++) { buf[i * 4] = 255; buf[i * 4 + 1] = 0; buf[i * 4 + 2] = 0; buf[i * 4 + 3] = 255; }
  diagLog("Buffer length: " + buf.length + " | expected (w*h*4): " + expected + " | match: " + (buf.length === expected));
  let layerId = null;
  const t0 = Date.now();
  try {
    await modal("Diag Test 1", async function() {
      layerId = await bpCreateLayer([{ _obj: "make", _target: [{ _ref: "layer" }], using: { _obj: "layer", name: "__DiagTest1" } }]);
...
async function diagTest2() {
  ...
  try {
    await modal("Diag Test 2", async function() {
      ...
      await bp([{ _obj: "set", _target: [{ _ref: "layer", _id: layerId }], to: { _obj: "layer", name: "__DiagTest2" } }]);
      const ta = Date.now();
      ...
      const w = px.imageData.width, h = px.imageData.height, comps = px.imageData.components;
      const expected = w * h * comps;
      diagLog("getPixels: " + (tb - ta) + "ms | bufferLength=" + buf.length + " | expected(w*h*components)=" + expected + " | match=" + (buf.length === expected));
      px.imageData.dispose();
      const tc = Date.now();
      const imgData = await window.imaging.createImageDataFromBuffer(buf, { width: w, height: h, components: comps, colorSpace: "RGB" });
      const td = Date.now();
...
async function diagTest3() {
  ...
  try {
    await modal("Diag Test 3", async function() {
      layerId = await bpCreateLayer([{ _obj: "mergeVisible", duplicate: true }]);
      await bp([{ _obj: "set", _target: [{ _ref: "layer", _id: layerId }], to: { _obj: "layer", name: "__DiagTest3" } }]);
      const ta = Date.now();
      const px = await window.imaging.getPixels({ layerID: layerId });
      const tb = Date.now();
      const buf = await px.imageData.getData();
      const w = px.imageData.width, h = px.imageData.height, comps = px.imageData.components;
      diagLog("getPixels: " + (tb - ta) + "ms | bufferLength=" + buf.length + " | expected=" + (w * h * comps) + " | match=" + (buf.length === w * h * comps));
      px.imageData.dispose();
      buf[0] = 0; buf[1] = 255; buf[2] = 0; if (comps >= 4) buf[3] = 255; // pixel (0,0) -> bright green
...
async function diagTest4() {
  ...
  try {
    await modal("Diag Test 4", async function() {
      layerId = await bpCreateLayer([{ _obj: "mergeVisible", duplicate: true }]);
      await bp([{ _obj: "set", _target: [{ _ref: "layer", _id: layerId }], to: { _obj: "layer", name: "__DiagTest4 (halftone result)" } }]);
      const ta = Date.now();
      const px = await window.imaging.getPixels({ layerID: layerId, targetSize: { width: 500, height: 500 } });
      const tb = Date.now();
      const buf = await px.imageData.getData();
      const w = px.imageData.width, h = px.imageData.height, comps = px.imageData.components;
      diagLog("getPixels: " + (tb - ta) + "ms | w=" + w + " h=" + h + " components=" + comps + " bufferLength=" + buf.length + " expected=" + (w * h * comps));
      px.imageData.dispose();
      ...
      const te = Date.now();
      const imgData = await window.imaging.createImageDataFromBuffer(out, { width: w, height: h, components: 4, colorSpace: "RGB" });
      const tf = Date.now();
    ...
    diagLog("TEST 4: SUCCESS \u2014 layer '__DiagTest4 (halftone result)' left in your document, check it visually, then delete it manually.");
    return true;
  } catch (e) {
...
async function runDiagnostics() {
  ...
  diagLog("");
  if (!ok1) { diagLog("STOPPED \u2014 Test 1 failed. The basic write path itself does not work; nothing past this point can be trusted."); setStatus("Diagnostics: Test 1 failed", "error"); return; }

  const ok2 = await diagTest2();
  diagLog("");
  if (!ok2) { diagLog("STOPPED \u2014 Test 2 failed. Writing works, but the read\u2192write round-trip is broken (likely a buffer/stride mismatch)."); setStatus("Diagnostics: Test 2 failed", "error"); return; }

  const ok3 = await diagTest3();
  diagLog("");
  if (!ok3) { diagLog("STOPPED \u2014 Test 3 failed. Round-trip works, but modify-then-write is broken."); setStatus("Diagnostics: Test 3 failed", "error"); return; }

  const ok4 = await diagTest4();
  diagLog("");
  if (ok4) {
    diagLog("ALL TESTS PASSED. The imaging read/write pipeline is sound at small scale.");
    diagLog("If the crash still happens on your real artwork, it is very likely resolution/performance related, not API misuse \u2014 check Test 4's stage timings above and compare against your real document size.");
    setStatus("Diagnostics: all tests passed", "success");
  } else {
    diagLog("Test 4 failed \u2014 the basic API works (Tests 1\u20133 passed) but the real halftone algorithm's output triggers a failure.");
