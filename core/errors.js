...
async function safeGetPixels(layer, rect = null) {
  ...
  try {
    if (!layer) {
      throw new PhotoneshopError("getPixels", `layer is null or undefined`);
    }
    ...
    const err = new PhotoneshopError("getPixels", `failed to read pixels from layer "${layerName}"`, e).withLayerState(
      layer
    );
    throw err;
  }
...
async function safePutPixels(layer, pixels, rect = null) {
  ...
  try {
    if (!layer) {
      throw new PhotoneshopError("putPixels", `layer is null or undefined`);
    }

    if (!pixels || pixels.length === 0) {
      throw new PhotoneshopError("putPixels", `pixels buffer is empty or null`);
    }
...
async function safeReadLayerPixels(layer, options = {}) {
  ...
  try {
    if (!layer) {
      throw new PhotoneshopError("readLayerPixels", `layer is null or undefined`);
    }
...
async function safeWriteLayerPixels(layer, pixels, width, height, options = {}) {
  ...
  try {
    if (!layer) {
      throw new PhotoneshopError("writeLayerPixels", `layer is null or undefined`);
    }

    if (!pixels || pixels.length === 0) {
      throw new PhotoneshopError("writeLayerPixels", `pixels buffer is empty`);
    }
...
function validateDocument(doc, operation) {
  if (!doc) {
    throw new PhotoneshopError(operation, `no active document. Open an image in Photoshop first.`);
  }
  ...
  if (mode !== "" && mode.indexOf("RGB") === -1) {
    throw new PhotoneshopError(
      operation,
      `document is not in RGB mode (current: ${raw}). ` + `Image > Mode > RGB to convert.`
    );
  }

  if (!doc.activeLayer) {
    throw new PhotoneshopError(operation, `no active layer. Select a layer before operating.`);
  }
...
function logError(err) {
  const msg = err instanceof PhotoneshopError ? err.toString() : `${err.name || "Error"}: ${err.message || err}`;
